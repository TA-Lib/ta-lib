...
| `stream.out_range()` | any time | the bars the stream has a value for — the batch range over the same bars |

```rust
let v = s.value();          // the value at the last committed bar
let mut fork = s.clone();   // independent from here on
let r = s.out_range();      // the bars s has a value for
```
...
function answers all of them at once. It is seeded by the opener, refreshed by
every accepted `update` and `update_and_fill`, and left alone by `peek`, so it
always names the bar `out_range()` reports.
...
`out_range()` reports the bars the stream has a value for. A stream opened over
`history.len()` bars starts at `(lookback, history.len() - lookback)`; each accepted
`update` adds one bar, and `peek` leaves it unchanged.
