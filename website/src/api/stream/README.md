...
| `TA_<NAME>_Update` / `TA_<NAME>_Peek` | `TA_BAD_PARAM` on NULL arguments, or invalid input such as NaN or ±Inf. A rejection changes nothing at all — no state, no output, and no range — so the next call sees exactly what the last accepted bar left. The exception is an overflow inside the library's own arithmetic, which is undefined in both APIs (see [Numerical Stability](/api/#numerical_stability)) — close such a handle rather than feeding it further. To count a rejected bar rather than re-feed it, call `TA_<NAME>_Advance` (see [Utility Calls](#utility-calls)).<br>`TA_<NAME>_Update` also reports `TA_OUT_OF_RANGE_END_INDEX` once the range has reached bar `TA_MAX_INDEX`, the last index the batch API addresses. That one does not clear: close the handle and open a new one on a shorter history. `TA_<NAME>_Peek` counts no bar and is not subject to it. |
