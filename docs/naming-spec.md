...
Status: **proposal under review**. Nothing here is implemented. The two tracked
halves are #172 D8 (Java) and #179 C16 (Rust); C# has no loose-ends issue and is
carried here.
...
A managed cell reads `today` -> `decided`, and `=` where today's spelling already
is the decision. The arrows go away once the rename lands, leaving one column per
backend.

Each backend's own convention decides, except where a row points at
...
Nothing above is new. #278 applied these folds to the 402 streaming entry points
and to all 201 handle types; it put the batch tier out of scope, and nobody has
come back to it. Sections 2 to 9 are mostly that unfinished pass plus the enum
and constant casings it never covered.

## 2. Per-function surface, 201 functions

| surface | C (frozen) | Rust | Java | C# |
|---|---|---|---|---|
| batch | `TA_SMA` | `SMA` -> `sma` | `SMA` -> `sma` | `SMA` -> `Sma` |
| lookback | `TA_SMA_Lookback` | `SMA_Lookback` -> `sma_lookback` | `SMA_Lookback` -> `smaLookback` | `SMA_Lookback` -> `SmaLookback` |
| numerics tier (not public) | body of `TA_SMA` | `SMA_Impl` -> `sma_impl` | `SMA_Impl` -> `smaImpl` | `SMA_Impl` -> `SmaImpl` |
...
The batch, lookback and enum-member rows are what #172 D8 and #179 C16 already
track. Everything else here is new.
...
|---|---|---|---|---|
| `RetCode` | `TA_SUCCESS`, `TA_BAD_PARAM` | `Success`, `BadParam` = | `Success`, `BadParam` -> `SUCCESS`, `BAD_PARAM` | `Success`, `BadParam` = |
| `CandleSettingType` | `TA_BodyLong` | `BodyLong` = | `BodyLong` -> `BODY_LONG` | `BodyLong` = |
| `RangeType` | `TA_RangeType_RealBody` | `RealBody` = | `RealBody` -> `REAL_BODY` | `RealBody` = |
| `MAType` | `TA_MAType_SMA` | `SMA` = (E2) | `SMA` = | `SMA` = (E2) |
| `FuncUnstId` | `TA_FUNC_UNST_HT_DCPERIOD` | `HT_DCPERIOD` = (E2) | `HT_DCPERIOD` = | `HT_DCPERIOD` = (E2) |

Java is the only backend whose enum constants are not in its own convention:
22 of them (7 `RetCode`, 12 `CandleSettingType`, 3 `RangeType`).
...
|---|---|---|---|---|
| defaults and bounds | `TA_REAL_DEFAULT`, `TA_INTEGER_DEFAULT`, `TA_MAX_INDEX` | `Core::REAL_DEFAULT`, `INTEGER_DEFAULT`, `MAX_INDEX` = | `Core.REAL_DEFAULT`, `INTEGER_DEFAULT`, `MAX_INDEX` = | `Core.REAL_DEFAULT`, `INTEGER_DEFAULT`, `MAX_INDEX` -> `RealDefault`, `IntegerDefault`, `MaxIndex` |

Seven C# constants: `REAL_DEFAULT`, `REAL_MIN`, `REAL_MAX`, `INTEGER_DEFAULT`,
`INTEGER_MIN`, `INTEGER_MAX`, `MAX_INDEX`. Screaming snake is the Java and Rust
rule and the C inheritance; .NET spells constants PascalCase.
...
| marker | n/a | n/a | `TaLibFailure` -> `TALibFailure` | `ITaLibFailure` -> `ITALibFailure` |
| bad argument | `TA_BAD_PARAM` | `RetCode::BadParam` = | `TaLibArgumentException` -> `TALibArgumentException` | `TaLibArgumentException` -> `TALibArgumentException` |
| bad index | `TA_OUT_OF_RANGE_*` | `RetCode::OutOfRange*` = | `TaLibIndexException` -> `TALibIndexException` | `TaLibArgumentOutOfRangeException` -> `TALibArgumentOutOfRangeException` |
| bad state | n/a | n/a | `TaLibStateException` -> `TALibStateException` | `TaLibInvalidOperationException` -> `TALibInvalidOperationException` |
...
| holder | `TA_ParamHolder` | `ParamHolder` = | `ParamHolder` = | `FunctionCall` -> `ParamHolder` |
| set input | `TA_SetInputParamRealPtr` | `set_input` = | `setInput` = | `SetInput` = |
| set parameter | `TA_SetOptInputParamInteger` | `set_opt` -> `set_opt_input` | `setOptInput` = | `SetOption`, `SetParam` -> `SetOptInput` |
| set output | `TA_SetOutputParamRealPtr` | `set_output` = | `setOutput` = | `SetOutput` = |
| lookback | `TA_GetLookback` | `lookback` = | `lookback` = | `Lookback` = |
| invoke | `TA_CallFunc` | `call` = | `call` = | `Invoke`, `TryInvoke` -> `Call`, `TryCall` |
| descriptor | `TA_FuncInfo` | `FuncInfo` = | `FunctionInfo` -> `FuncInfo` | `FunctionInfo` -> `FuncInfo` |
| flags | `TA_FuncFlags`, `TA_InputFlags` | `FuncFlags`, `InputFlags` = | `FuncFlags`, `InputFlags` = | `FunctionFlags` -> `FuncFlags` |

This is the one area where the backends disagreed about *words*, not casing.
The tier exists to mirror C, so it takes C's words and recases them:
`ParamHolder`, `setInput` / `setOptInput` / `setOutput`, `lookback`, `call`,
`FuncInfo`. C# pays most of the cost, losing `FunctionCall`, `SetOption` and
`Invoke`, which read better in isolation; `TryCall` keeps .NET's `Try` prefix
because that is a pattern, not a word.
...
| backend | identity | recommendation |
|---|---|---|
| C | `libta-lib`, `TA_` prefix | frozen |
