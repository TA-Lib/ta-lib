...
- New TA Functions:
  - MEDIAN: Rolling Median (proposal-drafts#73)
...
- (#410) RSI optimization with some floating-point re-ordering. On the 0-100 scale
  values changed by at most 5.7e-14 at the default period, growing slowly with the period
  (7.8e-14 at 100, 5.3e-13 at 20000) and unaffected by the price scale. STOCHRSI inherits
  the change, measured up to 1.1e-11 different than the legacy calculation.
...
- (#388) API: the MetaStock variant when the MAType is EMA, DEMA or TEMA is removed. Default behavior is unchanged. `TA_SetCompatibility()` and `TA_GetCompatibility()` remain, so existing sources still compile and link, but the setter now does nothing and the getter always answers `TA_COMPATIBILITY_DEFAULT`. Moving forward TA-Lib no longer supports "Compatibility Settings".
...
- (#395) On extreme inputs, CCI could return `+/-Inf`, CORREL a `NaN` or a perfect +/-1 correlation from a window that had none, and WILLR a value outside its documented [-100, 0] range. CCI and CORREL now answer such a window with 0, and WILLR is always in range, with a close on the period low coming out as exactly -100.
