...
RMA is also selectable as a moving-average type (`TA_MAType_RMA`) wherever an `optInMAType` parameter is accepted ([`MA`](/functions/ma), [`BBANDS`](/functions/bbands), [`STOCH`](/functions/stoch), [`MACDEXT`](/functions/macdext), ...).
