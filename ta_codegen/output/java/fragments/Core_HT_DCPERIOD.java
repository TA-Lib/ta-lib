 ...
 */
                               ...
                               double outReal[] )
   {
      ...
      int guardStart = clampedStart("HT_DCPERIOD", startIdx, htDcperiodLookback());
      ...
      RetCode retCode = htDcperiodImpl(startIdx, endIdx, inReal, outBegIdx, outNBElement, outReal);
...
/**** Streaming API *****/
   ...
   public static final class HtDcperiodStream {
       ...
       * <p>It is what {@link Core#htDcperiod} reports over the same bars: the
   ...
   private RetCode htDcperiodOpenImpl( HtDcperiodStream sp, double inReal[], int startIdx, MInteger outBegIdx, MInteger outNBElement, double outReal[], int outStride )
   {
      int outIdx = 0;
      int i = 0;
      int lookbackTotal = 0;
      ...
      while( today <= endIdx ) {
         ...
         smoothPeriod = Math.fma(0.67, smoothPeriod, 0.33 * period);
         if( today >= startIdx ) {
            outReal[outIdx++ * outStride] = smoothPeriod;
         }
         /* Ooof... let's do the next price bar now! */
         today += 1;
      }
   ...
   /* htDcperiodOpenAndFill anchored at startIdx — the composed-open fusion seam. */
   HtDcperiodStream htDcperiodOpenAndFillInternal( double inReal[], int startIdx, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      HtDcperiodStream sp = new HtDcperiodStream(this);
      RetCode retCode = htDcperiodOpenImpl(sp, inReal, startIdx, outBegIdx, outNBElement, outReal, 1);
   ...
   HtDcperiodStream htDcperiodOpenInternal( double inReal[], int startIdx )
   {
      ...
      RetCode retCode = htDcperiodOpenImpl(sp, inReal, startIdx, outBegIdx, outNBElement, sink_outReal, 0);
      sp.outRangeBegIdx = outBegIdx.value;
      sp.outRangeCount = outNBElement.value;
      if( retCode == RetCode.SUCCESS ) {
         return sp;
    ...
    * {@link HtDcperiodStream#outRange()}.
    */
   public HtDcperiodStream htDcperiodOpenAndFill( double inReal[], double outReal[] )
   {
      ...
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
      return htDcperiodOpenAndFillInternal(inReal, 0, outBegIdx, outNBElement, outReal);
   }
