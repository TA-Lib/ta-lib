...
/**** Streaming API *****/
   ...
   public static final class AdrStream {
      ...
      public void advance() {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("ADR advance", RetCode.OUT_OF_RANGE_END_INDEX);
         this.outRangeCount++;
      }
      ...
      public double update( double inHigh, double inLow ) {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("ADR update", RetCode.OUT_OF_RANGE_END_INDEX);
         if( !Double.isFinite(inHigh) || !Double.isFinite(inLow) )
            throw nonFiniteBar("ADR update", !Double.isFinite(inHigh) ? "inHigh" : "inLow");
      ...
      public double peek( double inHigh, double inLow ) {
         if( !Double.isFinite(inHigh) || !Double.isFinite(inLow) )
            throw nonFiniteBar("ADR peek", !Double.isFinite(inHigh) ? "inHigh" : "inLow");
         AdrStream sp = this;
         double tempReal = 0.0;
   ...
   AdrStream adrOpenAndFillInternal( double inHigh[], double inLow[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("ADR openAndFill", inHigh.length, startIdx, adrLookback(optInTimePeriod));
      }
   ...
   AdrStream adrOpenInternal( double inHigh[], double inLow[], int startIdx, int optInTimePeriod )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("ADR open", inHigh.length, startIdx, adrLookback(optInTimePeriod));
      }
   ...
   public AdrStream adrOpenAndFill( double inHigh[], double inLow[], int optInTimePeriod, double outReal[] )
   {
      ...
      if( (Object)outReal == (Object)inHigh || (Object)outReal == (Object)inLow ) {
         throw streamFailure("ADR openAndFill", RetCode.BAD_PARAM);
      }
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
