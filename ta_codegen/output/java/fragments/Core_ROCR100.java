 ...
 */
                            ...
                            double outReal[] )
   {
      ...
      int guardStart = clampedStart("ROCR100", startIdx, rocr100Lookback(optInTimePeriod));
      ...
      RetCode retCode = rocr100Impl(startIdx, endIdx, inReal, optInTimePeriod, outBegIdx, outNBElement, outReal);
...
/**** Streaming API *****/
   ...
   public static final class Rocr100Stream {
      ...
      public void advance() {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("ROCR100 advance", RetCode.OUT_OF_RANGE_END_INDEX);
         this.outRangeCount++;
      }
      ...
      public double update( double inReal ) {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("ROCR100 update", RetCode.OUT_OF_RANGE_END_INDEX);
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("ROCR100 update", "inReal");
      ...
      public double peek( double inReal ) {
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("ROCR100 peek", "inReal");
   ...
   Rocr100Stream rocr100OpenAndFillInternal( double inReal[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("ROCR100 openAndFill", inReal.length, startIdx, rocr100Lookback(optInTimePeriod));
      }
   ...
   Rocr100Stream rocr100OpenInternal( double inReal[], int startIdx, int optInTimePeriod )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("ROCR100 open", inReal.length, startIdx, rocr100Lookback(optInTimePeriod));
      }
      throw streamFailure("ROCR100 open", retCode);
   }
   /**
   ...
   public Rocr100Stream rocr100OpenAndFill( double inReal[], int optInTimePeriod, double outReal[] )
   {
      ...
      if( (Object)outReal == (Object)inReal ) {
         throw streamFailure("ROCR100 openAndFill", RetCode.BAD_PARAM);
      }
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
