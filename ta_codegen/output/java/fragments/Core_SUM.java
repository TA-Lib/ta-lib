 ...
 */
                        ...
                        double outReal[] )
   {
      ...
      int guardStart = clampedStart("SUM", startIdx, sumLookback(optInTimePeriod));
      ...
      RetCode retCode = sumImpl(startIdx, endIdx, inReal, optInTimePeriod, outBegIdx, outNBElement, outReal);
...
/**** Streaming API *****/
   ...
   public static final class SumStream {
      ...
      public void advance() {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("SUM advance", RetCode.OUT_OF_RANGE_END_INDEX);
         this.outRangeCount++;
      }
      ...
      public double update( double inReal ) {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("SUM update", RetCode.OUT_OF_RANGE_END_INDEX);
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("SUM update", "inReal");
      ...
      public double peek( double inReal ) {
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("SUM peek", "inReal");
         SumStream sp = this;
         double tempReal = 0.0;
   ...
   SumStream sumOpenAndFillInternal( double inReal[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("SUM openAndFill", inReal.length, startIdx, sumLookback(optInTimePeriod));
      }
      throw streamFailure("SUM openAndFill", retCode);
   }
   ...
   SumStream sumOpenInternal( double inReal[], int startIdx, int optInTimePeriod )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("SUM open", inReal.length, startIdx, sumLookback(optInTimePeriod));
      }
      throw streamFailure("SUM open", retCode);
   }
   /**
   ...
   public SumStream sumOpenAndFill( double inReal[], int optInTimePeriod, double outReal[] )
   {
      ...
      if( (Object)outReal == (Object)inReal ) {
         throw streamFailure("SUM openAndFill", RetCode.BAD_PARAM);
      }
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
