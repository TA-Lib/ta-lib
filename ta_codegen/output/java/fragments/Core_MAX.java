 ...
 */
                        ...
                        double outReal[] )
   {
      ...
      int guardStart = clampedStart("MAX", startIdx, maxLookback(optInTimePeriod));
      ...
      RetCode retCode = maxImpl(startIdx, endIdx, inReal, optInTimePeriod, outBegIdx, outNBElement, outReal);
...
/* Using max_ALT1 for TA_ALT={STREAM,ALL_LANGUAGES} */
   ...
   public static final class MaxStream {
      ...
      public void advance() {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("MAX advance", RetCode.OUT_OF_RANGE_END_INDEX);
         this.outRangeCount++;
      }
      ...
      public double update( double inReal ) {
         if( this.outRangeBegIdx + this.outRangeCount > INDEX_MAX )
            throw failure("MAX update", RetCode.OUT_OF_RANGE_END_INDEX);
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("MAX update", "inReal");
      ...
      public double peek( double inReal ) {
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("MAX peek", "inReal");
         MaxStream sp = this;
         double tmp = 0.0;
   ...
   MaxStream maxOpenAndFillInternal( double inReal[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("MAX openAndFill", inReal.length, startIdx, maxLookback(optInTimePeriod));
      }
   ...
   MaxStream maxOpenInternal( double inReal[], int startIdx, int optInTimePeriod )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("MAX open", inReal.length, startIdx, maxLookback(optInTimePeriod));
      }
   ...
   public MaxStream maxOpenAndFill( double inReal[], int optInTimePeriod, double outReal[] )
   {
      ...
      if( (Object)outReal == (Object)inReal ) {
         throw streamFailure("MAX openAndFill", RetCode.BAD_PARAM);
      }
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
