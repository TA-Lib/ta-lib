...
/**** Streaming API *****/
   ...
   public static final class VhfStream {
      ...
      public double update( double inReal ) {
         ...
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("VHF update", "inReal");
      ...
      public double peek( double inReal ) {
         if( !Double.isFinite(inReal) )
            throw nonFiniteBar("VHF peek", "inReal");
         VhfStream sp = this;
         int i = 0;
         double highest = 0.0;
         double lowest = 0.0;
   ...
   VhfStream vhfOpenAndFillInternal( double inReal[], int startIdx, int optInTimePeriod, MInteger outBegIdx, MInteger outNBElement, double outReal[] )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("VHF openAndFill", inReal.length, startIdx, vhfLookback(optInTimePeriod));
      }
      throw streamFailure("VHF openAndFill", retCode);
   }
   ...
   VhfStream vhfOpenInternal( double inReal[], int startIdx, int optInTimePeriod )
   {
      ...
      if( retCode == RetCode.INSUFFICIENT_HISTORY ) {
         throw insufficientHistory("VHF open", inReal.length, startIdx, vhfLookback(optInTimePeriod));
      }
      throw streamFailure("VHF open", retCode);
   }
   /**
   ...
   public VhfStream vhfOpenAndFill( double inReal[], int optInTimePeriod, double outReal[] )
   {
      ...
      if( (Object)outReal == (Object)inReal ) {
         throw streamFailure("VHF openAndFill", RetCode.BAD_PARAM);
      }
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
