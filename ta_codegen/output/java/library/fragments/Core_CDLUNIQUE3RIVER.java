 ...
 */
   ...
   RetCode cdlUnique3RiverUnguardedInternal( int startIdx,
                                             int endIdx,
                                             double inOpen[],
                                             double inHigh[],
                                             double inLow[],
                                             ...
                                             int outInteger[] )
   {
      MInteger outBegIdx = new MInteger();
      MInteger outNBElement = new MInteger();
      cdlUnique3RiverUnguardedInternal(startIdx, endIdx, inOpen, inHigh, inLow, inClose, outBegIdx, outNBElement, outInteger);
      return new OutRange(outBegIdx.value, outNBElement.value);
   }
   /**
    * A three-candle bullish reversal pattern: a long black candle, then a black
...
/**** Streaming API *****/
   ...
   void cdlUnique3RiverStreamStep( CdlUnique3RiverStream sp, double inOpen, double inHigh, double inLow, double inClose )
   {
      ...
      if( ((sp.lag2_inClose >= sp.lag2_inOpen) ? 1 : 0 - 1) == 0 - 1 && /* black */
          ...
          Math.abs(inClose - inOpen) < ((BodyShort_factor * (((BodyShort_avgPeriod != 0) ? (sp.BodyShortPeriodTotal / BodyShort_avgPeriod) : ((BodyShort_rangeType == 0) ? (Math.abs(inClose - inOpen)) : ((BodyShort_rangeType == 1) ? (inHigh - inLow) : ((BodyShort_rangeType == 2) ? ((inHigh - inLow) - Math.abs(inClose - inOpen)) : 0.0)))) / ((BodyShort_rangeType == 2) ? 2.0 : 1.0)))) ) /* 3rd: short */
      {
