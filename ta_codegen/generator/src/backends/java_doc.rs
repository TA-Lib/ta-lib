...
pub fn guarded_docs(
    ...
    b.tag(
        ...
        "if an optional parameter is outside its documented range, two outputs share \
         one array, or an array is absent or too short for the range requested — an \
         input this function <i>reads</i> that does not reach {@code endIdx}, or an \
         output that cannot hold the values produced. A few candlestick patterns \
         declare an OHLC series they never index; those are neither length-checked \
         nor null-checked, because rejecting them would refuse a call the algorithm \
         can answer. Checked before anything is written, so a rejected call leaves \
