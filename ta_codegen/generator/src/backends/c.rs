...
/// Words this backend cannot render as an identifier (see
/// [`crate::naming`]). Two groups:
///
/// 1. The C keywords, through C23. A local named `restrict` or `_Atomic` would
///    not compile.
/// 2. The C++-only keywords and alternative operator tokens. The generated
///    argument names reach `include/ta_func.h`, whose prototypes sit inside an
///    `extern "C"` block — so a parameter named `class` or `not` compiles fine
///    as C and breaks every C++ consumer of the public header.
pub(crate) const RESERVED_WORDS: &[&str] = &[
    // --- C keywords (C89/C99/C11/C23) ---
    "alignas",
    "alignof",
    "auto",
    "bool",
    ...
    "_Static_assert",
    "_Thread_local",
    // --- C++-only, for the `extern "C"` public header ---
    "and",
    "and_eq",
    "asm",
    "bitand",
        ...
        match &opt.param_type {
            ...
            ParamType::Real => {
                if let Some(default_val) = opt.default {
                    ...
                    // Every declared bound is checked, including the ±3e37
                    // sentinels — that is what the pre-cutover reference emitted,
