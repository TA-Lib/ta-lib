...
//! `div_open_impl` carries its own transcription of the batch body (the warm-up
...
//! whole table goes through `div_open_and_fill`, and again through `peek`/
...
fn the_fill_loop_agrees_on_every_row() {
    // div_open_impl, not div_step_impl: a separate transcription of the batch
    ...
    let (stream, range) = core
        .div_open_and_fill(&num, &den, &mut out)
        .expect("a zero divisor is not an error");
    assert_eq!(range.beg_idx, 0);
